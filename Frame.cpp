...
std::future<void> Frame::apply(std::vector<std::unique_ptr<Algorithm>> &algorithms)
{
	return std::async(	[](std::vector<std::unique_ptr<Algorithm>> &&algorithms, Image *image)
						{
							std::vector<std::unique_ptr<Algorithm>> unapplied = std::move(algorithms);
